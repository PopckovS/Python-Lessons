...
def func9():
    """
    Пример работы с тем как ведется подсчет ссылок на обьекты,
    id() водит уникальное место в памяти обьекта.
    """
    def test_object_links():
        x = 300
        y = 300
        show(x, y, 'Сравнение чисел int')

        x = 30000000000000000000000000000000000000000000000.65
        y = 30000000000000000000000000000000000000000000000.65
        show(x, y, 'Сравнение чисел float')

        x = 'name'
        y = 'name'
        show(x, y, 'Сравнение строк')

        x = (300, 300)
        y = (300, 300)
        show(x, y, 'Сравнение кортежей')
        print('id(x[0]), id(x[1]) :', id(x[0]), id(x[1]))
        print('id(y[0]), id(y[1]) :', id(y[0]), id(y[1]))
