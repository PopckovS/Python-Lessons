...
`__init__.py` - Пустой файл что указывает что даанная директория, должна 
расматриваться как пакет питона.

`settings.py` - файл конфигов, является ангалогом файла .env из Symfony.
...
Приложение - отдельный модуль который имеет свою внутреню сеть URL адресов,
